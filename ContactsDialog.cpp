...
void ContactsDialog::onComboBoxSelected(){
    ...
    ui->comboBox->setCurrentIndex(1); ui->comboBox->addItem("Поиск по номеру телефона");
    ui->comboBox->setCurrentIndex(2); ui->comboBox->addItem("Поиск по заметке");

}

void ContactsDialog::showTable(){
...
//         QString strFilterPhone = QString("entry_phone LIKE '%%1%'").arg(entry_phone);
//         model.setFilter(strFilterPhone);
//         model.setSort(0, Qt::DescendingOrder);
//         model.select();
